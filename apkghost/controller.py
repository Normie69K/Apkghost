...
class Controller:
    ...
    def _build_apk_analyzer_tab(self, tab):
        ...
        self.out_path_label = ctk.CTkLabel(control_frame, text="No output directory selected...", anchor="w", fg_color="#333333", corner_radius=6, padx=10); self.out_path_label.grid(row=1, column=1, sticky="ew", pady=(10, 0))
        self.engine_var = ctk.StringVar(value="apktool"); engine_menu = ctk.CTkOptionMenu(control_frame, variable=self.engine_var, values=["apktool", "androguard"]); engine_menu.grid(row=0, column=2, padx=(20,10)); ctk.CTkLabel(control_frame, text="Engine").grid(row=0, column=3, padx=(0,10))
        self.analyze_btn = ctk.CTkButton(control_frame, text="Decompile & Analyze", fg_color="green", state="disabled", command=self.on_analyze); self.analyze_btn.grid(row=1, column=2, columnspan=2, padx=(20, 0), pady=(10, 0), ipady=10, sticky="ew")
        ...
        self.export_btn = ctk.CTkButton(bottom_frame, text="Export Report as PDF", state="disabled", command=self.on_export_pdf); self.export_btn.grid(row=0, column=1, sticky="e")
    ...
    def _build_file_inspector_tab(self, tab):
        tab.grid_rowconfigure(1, weight=1); tab.grid_columnconfigure(0, weight=1)
        inspector_btn = ctk.CTkButton(tab, text="Select Any File to Inspect...", command=self.on_inspect_file); inspector_btn.grid(row=0, column=0, padx=20, pady=20, sticky="ew")
        self.inspector_view = ctk.CTkTextbox(tab, font=("monospace", 13)); self.inspector_view.grid(row=1, column=0, padx=20, pady=(0, 20), sticky="nsew")
    ...
    def on_export_pdf(self):
        ...
        if not p: return
        try:
            doc = SimpleDocTemplate(p); s = getSampleStyleSheet(); story = [Paragraph("Apkghost Report", s['h1'])]
            story.append(Paragraph(self.report_text.replace("\n", "<br/>"), s['Code']))
    ...
    def on_inspect_file(self):
        ...
        if not p: return
        self.inspector_view.delete("1.0", "end"); self.set_status(f"Inspecting..."); self.tab_view.set("File Inspector")
    ...
    def _analysis_job(self):
        ...
        self.log(f"Starting decompilation with {self.engine_var.get()}...")
        ...
        self.log(r["output"])
        if not r["success"]: self.log("Decompilation failed."); self.set_status("Decompilation Failed"); self.set_ui_state("normal"); return
        self.log("Decompilation successful."); self.app.after(0, self.populate_tree, self.file_tree, self.output_dir)
        self.set_status("Running static analysis..."); self.log("\n-> Starting static analysis...")
    ...
    def _inspect_job(self, path):
        r = inspect_file(path); self.app.after(0, self.inspector_view.insert, "1.0", r); self.set_status("Inspection Complete")
    ...
    def format_report(self, results):
        ...
        hints = results.get('crypto_hints', []); report.append("\n".join(f"  [!] Found potential {h}." for h in hints) or "  No signs of strong crypto found.\n")
        report.append("\n---[ Decoded Strings ]---")
        decoded = results.get('decoded_strings', [])
        if decoded: [report.extend([f"  [+] Decoded: {i['decoded']}", f"      Original: {i['encoded'][:50]}...", f"      In File: {i['file']}\n"]) for i in decoded]
        ...
        else: report.append("  No hardcoded secrets found.\n")
        return "\n".join(report)
    ...
    def populate_tree(self, tree, path):
        ...
        except FileNotFoundError: self.log(f"Dir not found: {path}")
